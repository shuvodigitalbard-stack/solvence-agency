  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h1 style={{ fontSize: '1.8rem', fontFamily: 'Red Hat Display', fontWeight: 900, color: '#1a1a2e' }}>Messages</h1>
        ...
        {messages.map(m => (
          <div key={m._id} className="card" style={{ padding: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
              <div><span style={{ fontWeight: 600, color: '#1a1a2e' }}>{m.name}</span><span style={{ color: '#64748b', fontSize: '0.85rem', marginLeft: '12px' }}>{m.email}</span></div>
            ...
            {m.subject && <div style={{ color: '#4a5568', fontSize: '0.85rem', marginBottom: '8px' }}>Subject: {m.subject}</div>}
            <p style={{ color: '#334155', lineHeight: 1.7, marginBottom: '12px' }}>{m.message}</p>
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              <span style={{ color: '#94a3b8', fontSize: '0.75rem' }}>{new Date(m.createdAt).toLocaleString()}</span>
              <div style={{ flex: 1 }} />
