  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="card" style={{ marginBottom: '32px' }}>
        ...
        <div style={{ marginBottom: '16px' }}><label style={{ display: 'block', marginBottom: '4px', color: '#4a5568', fontSize: '0.85rem' }}>Notes</label><textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} rows={2} /></div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {clients.map(c => (
          <div key={c._id} className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px' }}>
            <div><div style={{ fontWeight: 600, color: '#1a1a2e' }}>{c.name}</div><div style={{ color: '#64748b', fontSize: '0.8rem' }}>{c.company} • {c.status} • ${c.projectValue?.toLocaleString()}</div></div>
