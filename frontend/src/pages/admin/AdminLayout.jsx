...
import { FiHome, FiBriefcase, FiUsers, FiMessageSquare, FiSettings, FiLogOut } from 'react-icons/fi';
...
export default function AdminLayout() {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f8f9fa' }}>
      <aside style={{ width: '240px', background: '#1a1a2e', borderRight: '1px solid rgba(255,255,255,0.05)', padding: '24px 0', display: 'flex', flexDirection: 'column' }}>
        ...
        <nav style={{ flex: 1 }}>
          {navItems.map(item => (
            <Link key={item.to} to={item.to} style={{
              ...
              color: location.pathname === item.to ? '#e0c060' : '#64748b',
              background: location.pathname === item.to ? 'rgba(224,192,96,0.1)' : 'transparent',
              borderRight: location.pathname === item.to ? '3px solid #e0c060' : '3px solid transparent',
        ...
        <div style={{ padding: '0 20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '12px 0', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'var(--gradient)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.8rem', color: '#fff' }}>{user?.name?.charAt(0)}</div>
